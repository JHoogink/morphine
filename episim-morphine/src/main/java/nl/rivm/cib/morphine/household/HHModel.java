...
public class HHModel implements Scenario {
	...
	public void init() throws ParseException, InstantiationException, IllegalAccessException, IOException {
		...
		this.hhAttributes = SparseMatrix.Factory.zeros(n, HHAttribute.values().length);
		this.ppAttributes = SparseMatrix.Factory.zeros(3 * n, HHMemberAttribute.values().length);
		this.hhPressure = SparseMatrix.Factory.zeros(3 * n, 3 * n);

		this.timeRange = Range.upFromAndIncluding(scheduler().offset().toLocalDate());
		this.localHouseholdDist = ConditionalDistribution.of(this.distFactory::createCategorical,
		...
		this.initial = JsonUtil.valueOf(FileUtil.toInputStream("conf/hesitancy-initial.json"), float[][].class);

		// populate households
		for (long time = System.currentTimeMillis(), i = 0, agPrev = 0; i < n; i++) {
			createHousehold();
			if (System.currentTimeMillis() - time > 1000) {
				time = System.currentTimeMillis();
				long agNow = this.persons.get() + this.households.get();
				LOG.info("Initialized {} pp ({}%) across {} hh (= +{} actors/sec)", this.persons.get(),
						this.persons.get() * 100 / n, this.households.get(), agNow - agPrev);
				agPrev = agNow;
			}
		}
		...
		LOG.info("Initialized {} pp ({}%) across {} hh", this.persons.get(), this.persons.get() * 100 / n,
				this.households.get());

		// final Pathogen measles = this.pathogens.create( "MV-1" );

		atEach(this.config.statisticsRecurrence(scheduler())).subscribe(this::exportStatistics, this::logError);
	}
	...
	private static final HHAttribute[] CHILD_REF_COLUMN_INDICES = {
			...
			HHAttribute.CHILD1_REF, HHAttribute.CHILD2_REF, HHAttribute.CHILD3_REF };
	...
	private void vaccinate(final Instant t) {
		...
		final Range<BigDecimal> birthRange = Range.of(nowYears.subtract(BigDecimal.valueOf(4)), nowYears);
		LOG.info("t={}, vax members positive on {}, susceptible, born {}",
				t.prettify(scheduler().offset().toLocalDate()), occ, birthRange);
		...
		this.attitudeEvaluator.isPositive(occ, this.hhAttributes)
				...
				.forEach(hhRef -> Arrays.stream(CHILD_REF_COLUMN_INDICES)
						.mapToLong(hhAtt -> this.hhAttributes.getAsLong(hhRef, hhAtt.ordinal()))
						...
						// then vaccinate
						.forEach(ppRef -> vaccinate(hhRef, ppRef, birthRange)));
	}

	private void vaccinate(final long hhRef, final long ppRef, final Range<BigDecimal> birthRange) {
		LOG.info("Vax in (positive) hh #{} (susceptible) pp #{} born {}...", hhRef, ppRef, birthRange);
	}
	...
	private int createHousehold() {
		...
		final Quantity<Time> hhRefAge = hhCat.ageDist(this.distFactory::createUniformContinuous).draw();
		...
		final Actor.ID hhRef = Actor.ID.of(String.format("hh%08d", id), this.binder.id());
		...
		final HHMemberStatus hhStatus = hesProf.status != VaccineStatus.none ? HHMemberStatus.IMMUNE
		...
		final long partnerRef = hhType.adultCount() < 2 ? NA
				: createPerson(hhRefAge.subtract(QuantityUtil.valueOf(3, TimeUnits.ANNUM)), hhStatus);
		final long child1Ref = hhType.childCount() < 1 ? NA
				: createPerson(hhRefAge.subtract(QuantityUtil.valueOf(20, TimeUnits.ANNUM)), hhStatus);
		final long child2Ref = hhType.childCount() < 2 ? NA
				: createPerson(hhRefAge.subtract(QuantityUtil.valueOf(22, TimeUnits.ANNUM)), hhStatus);
		final long child3Ref = hhType.childCount() < 3 ? NA
				: createPerson(hhRefAge.subtract(QuantityUtil.valueOf(24, TimeUnits.ANNUM)), hhStatus);

		// create non-NA household members pressure network, adults impress all
		final long[] impressedRefs = Arrays
				.stream(new long[] { referentRef, partnerRef, child1Ref, child2Ref, child3Ref })
				.filter(ref -> ref != NA).toArray();
		final long[] expressingRefs = Arrays.stream(new long[] { referentRef, partnerRef }).filter(ref -> ref != NA)
				.toArray();
		for (long impressedRef : impressedRefs)
			for (long expressingRef : expressingRefs)
				this.hhPressure.setAsBigDecimal(BigDecimal.ONE, impressedRef, expressingRef);

		final BigDecimal initialCalculation = this.calculationDist.draw();
		this.hesDist = () -> {
			final int row = this.distFactory.getStream().nextInt(this.initial.length);
			final HesitancyProfileJson hes = this.hesitancyDist.draw();
			return new float[] { this.initial[row][hes.indices.get(HesitancyDimension.confidence)-1],
					this.initial[row][hes.indices.get(HesitancyDimension.complacency)-1] };
		};
...
//				.valueOf(hesProf.distParams.get(HesitancyDimension.complacency).createDist(this.distFactory).draw());
		...
		this.hhAttributes.setAsLong(placeIndex, hhIndex, HHAttribute.HOME_REF.ordinal());
		this.hhAttributes.setAsBoolean(hesProf.religious, hhIndex, HHAttribute.RELIGIOUS.ordinal());
		this.hhAttributes.setAsBoolean(hesProf.alternative, hhIndex, HHAttribute.ALTERNATIVE.ordinal());
		this.hhAttributes.setAsBoolean(hhType.registered(), hhIndex, HHAttribute.REGISTERED.ordinal());
		this.hhAttributes.setAsBigDecimal(initialCalculation, hhIndex, HHAttribute.CALCULATION.ordinal());
		this.hhAttributes.setAsFloat(initial[0], hhIndex, HHAttribute.CONFIDENCE.ordinal());
		this.hhAttributes.setAsFloat(initial[1], hhIndex, HHAttribute.COMPLACENCY.ordinal());
		this.hhAttributes.setAsLong(referentRef, hhIndex, HHAttribute.REFERENT_REF.ordinal());
		this.hhAttributes.setAsLong(partnerRef, hhIndex, HHAttribute.PARTNER_REF.ordinal());
		this.hhAttributes.setAsLong(child1Ref, hhIndex, HHAttribute.CHILD1_REF.ordinal());
		this.hhAttributes.setAsLong(child2Ref, hhIndex, HHAttribute.CHILD2_REF.ordinal());
		this.hhAttributes.setAsLong(child3Ref, hhIndex, HHAttribute.CHILD3_REF.ordinal());
	...
	private long createPerson(final Quantity<Time> initialAge, final HHMemberStatus status) {
		final long id = this.persons.incrementAndGet();
		final Actor.ID memberRef = Actor.ID.of(String.format("ind%08d", id), this.binder.id());
	...
	private long toHouseholdIndex(final Actor.ID hhRef) {
		return this.hhIndex.computeIfAbsent(hhRef, key -> (long) this.hhIndex.size());
	}

	private final Map<Actor.ID, Long> ppIndex = new HashMap<>();

	private long toMemberIndex(final Actor.ID hhRef) {
		return this.ppIndex.computeIfAbsent(hhRef, key -> (long) this.ppIndex.size());
	}
	...
	private void exportStatistics(final Instant t) {
		JPAUtil.session(this.emf).subscribe(
				em -> this.hhIndex.forEach((id,
						index) -> em.persist(HHStatisticsDao.create(id.contextRef(), t, this.hhAttributes, index,
								this.ppAttributes))),
				this::logError, () -> LOG.info("t = {} ({}), persisted {} households", t.prettify(TimeUnits.DAYS, 1),
