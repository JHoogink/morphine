...
public class MorphineMain1M implements DemoScenario
{
	...
	public void init() throws Exception
	{
		...
		this.data
				.withSource( map -> map.put( Persons.PersonTuple.class,
						Persons.PROPERTIES ), this.persons )
				.withSource( map -> map.put( Households.HouseholdTuple.class,
						Households.PROPERTIES ), this.households )
				.withSource(
						map -> map.put( Regions.RegionTuple.class,
								Regions.PROPERTIES ),
						(MapFactory<Long>) HashMap::new )
				.withSource(
						map -> map.put( Societies.SocietyTuple.class,
								Societies.PROPERTIES ),
						(MapFactory<Long>) HashMap::new )
				.withSource(
						map -> map.put( Sites.SiteTuple.class,
								Sites.PROPERTIES ),
						(MapFactory<Long>) HashMap::new );
